"""Training service for CLV regression models."""
...
class CLVTrainingService:
    ...
    def train(self, customers: list[dict[str, Any]]) -> dict[str, Any]:
        rows = [
            {"customer_id": customer.get("customer_id"), "target": customer.get("target_clv", 0)}
            for customer in customers
        ]
        model = self.trainer.train(rows, task_type="regression")
        return {
            **model,
